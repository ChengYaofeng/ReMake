...
class ClearGraspRealWorld(Dataset):
    ...
    def __getitem__(self, id):
        ...
        return process_data(rgb, depth, depth_gt, depth_gt_mask, camera_intrinsics, scene_type = "cluttered",
                            camera_type = 0, split = self.split, image_size = self.image_size, depth_min = self.depth_min,
                            depth_max = self.depth_max, depth_norm = self.depth_norm, use_aug = False, reldepth_model=self.reldepth_model)
...
class ClearGraspSynthetic(Dataset):
    ...
    def __getitem__(self, id):
        ...
        rgb = cv2.resize(rgb, self.image_size, interpolation = cv2.INTER_LINEAR)
        depth = cv2.resize(depth_gt, self.image_size, interpolation = cv2.INTER_NEAREST) #这里进行了修改
        depth_gt = cv2.resize(depth_gt, self.image_size, interpolation = cv2.INTER_NEAREST)
        depth_gt_mask = cv2.resize(depth_gt_mask, self.image_size, interpolation = cv2.INTER_NEAREST)
        depth_gt_mask = depth_gt_mask.astype(bool)

        return process_data(rgb, depth, depth_gt, depth_gt_mask, camera_intrinsics,
                            scene_type = "cluttered", camera_type = 0, split = self.split, image_size = self.image_size,
                            depth_min = self.depth_min, depth_max = self.depth_max, depth_norm = self.depth_norm,
                            use_aug = self.use_aug, rgb_aug_prob = self.rgb_aug_prob, use_depth_aug = self.use_depth_aug,
                            reldepth_model = self.reldepth_model)
