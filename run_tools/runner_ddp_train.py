...
from .ddp_tools import clean_state_dict, load_model_weights
...
def ddp_train(local_rank, args):
    ...
    model = builder.get_model().to(local_rank)
    ...
    if local_rank == 0 and os.path.isfile(checkpoint_file):
        checkpoint = torch.load(checkpoint_file, map_location=f'cuda:{local_rank}')
    ...
    # if local_rank == 0:
    reldepth_model = load_reldepth_model(args.reldepth_model, local_rank)
    # else:
    #     reldepth_model = None
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        with tqdm(train_dataloader) as pbar:
            for data_dict in pbar:
                optimizer.zero_grad()
                data_dict = to_device(data_dict, local_rank)
        ...
        # save model
        if is_main:
            state_dict = model.module.state_dict() if builder._multigpu() else model.state_dict()
